...
fn main() {
    ...
    let out_dir_c = PathBuf::from(&cargo_manifest_dir).join("src/s1ap/c");
    let out_dir_rs = PathBuf::from(&cargo_manifest_dir).join("src/s1ap/rs");

    // Generate the codec source files from the ASN.1 specification.
    Command::new("asn1c")
        .args(&[
            "asn1c/s1ap-r17.3.0/36413-h30.asn",
            "-D",
            out_dir_c.to_str().unwrap(),
            "-fcompound-names",
            "-no-gen-example",
        ])
        .status()
    ...
    let (sources, headers) = find_generated_sources_and_headers(&out_dir_c);

    let mut cc_builder = cc::Build::new();
    cc_builder.include(&out_dir_c.to_str().unwrap());
    ...
    let mut builder = bindgen::Builder::default()
        .clang_arg(format!("-I{}", &out_dir_c.to_str().unwrap()))
        .header("sys/types.h")
        .header("stdio.h");
    ...
    let mut f =
        File::create(out_dir_rs.join("bindings.rs")).expect("Unable to create file bindings.rs");
